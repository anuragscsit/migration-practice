...
class BranchesControllerTest < ActionDispatch::IntegrationTest
  ...
  test "should create branch" do
    assert_difference('Branch.count') do
      post branches_url, params: { branch: { branch_code: @branch.branch_code, name: @branch.name } }
    end
  ...
  test "should update branch" do
    patch branch_url(@branch), params: { branch: { branch_code: @branch.branch_code, name: @branch.name } }
